@echo off
REM ###########################################################################
REM # Validate the pinaxai library using ruff and mypy
REM # Usage: libs\pinaxai\scripts\validate.bat
REM ###########################################################################
...
ECHO.
ECHO ##################################################
ECHO # Running: ruff check %PINAXAI_DIR%
ECHO ##################################################
ECHO.

python -m ruff check "%PINAXAI_DIR%"
IF %ERRORLEVEL% NEQ 0 (
...
ECHO.
ECHO ##################################################
ECHO # Running: mypy %PINAXAI_DIR% --config-file %PINAXAI_DIR%\pyproject.toml
ECHO ##################################################
ECHO.

python -m mypy "%PINAXAI_DIR%" --config-file "%PINAXAI_DIR%\pyproject.toml"
IF %ERRORLEVEL% NEQ 0 (
    ECHO [ERROR] mypy validation failed with exit code %ERRORLEVEL%
    EXIT /B %ERRORLEVEL%
)

ECHO [INFO] Pinaxai validation complete.
EXIT /B 0 