...
def test_firecrawl_knowledge_base_directory(setup_vector_db):
    ...
    kb = FireCrawlKnowledgeBase(
        urls=["https://docs.pinax.tech/knowledge/introduction", "https://docs.pinax.tech/knowledge/pdf"],
    ...
    response = agent.run("What are knowledge bases in Pinaxai and what types are available?", markdown=True)
...
def test_firecrawl_knowledge_base_single_url(setup_vector_db):
    ...
    kb = FireCrawlKnowledgeBase(urls=["https://docs.pinax.tech/knowledge/pdf"], vector_db=setup_vector_db)
    kb.load(recreate=True)
    ...
    agent = Agent(knowledge=kb)
    response = agent.run("How do I use PDFKnowledgeBase in Pinaxai?", markdown=True)
...
async def test_firecrawl_knowledge_base_async_directory(setup_vector_db):
    ...
    response = await agent.arun(
        "What are the different types of knowledge bases available in Pinaxai and how do I use PDF knowledge base?",
        markdown=True,
    )
...
async def test_firecrawl_knowledge_base_async_single_url(setup_vector_db):
    ...
    kb = FireCrawlKnowledgeBase(urls=["https://docs.pinax.tech/knowledge/introduction"], vector_db=setup_vector_db)
    await kb.aload(recreate=True)
    ...
    agent = Agent(knowledge=kb)
    response = await agent.arun("What is a knowledge base in Pinaxai?", markdown=True)
