...
def test_website_knowledge_base_directory(setup_vector_db):
    ...
    urls = ["https://docs.pinax.tech/introduction/agents", "https://docs.pinax.tech/introduction/playground"]
    ...
    response = agent.run("What are agents in Pinaxai and what levels are there?", markdown=True)
...
def test_website_knowledge_base_single_url(setup_vector_db):
    ...
    kb = WebsiteKnowledgeBase(
        urls=["https://docs.pinax.tech/introduction/agents"], max_links=1, vector_db=setup_vector_db
    )
    kb.load(recreate=True)
    ...
    agent = Agent(knowledge=kb)
    response = agent.run("How do I create a basic agent in Pinaxai?", markdown=True)
...
async def test_website_knowledge_base_async_directory(setup_vector_db):
    ...
    response = await agent.arun("What are agents in Pinaxai and what levels are there?", markdown=True)
...
async def test_website_knowledge_base_async_single_url(setup_vector_db):
    ...
    response = await agent.arun("How do I create a basic agent in Pinaxai?", markdown=True)
