...
from pinaxai.storage.singlestore import SingleStoreStorage
...
def agent_storage(mock_engine, mock_session):
    ...
    with patch("pinaxai.storage.singlestore.sessionmaker", return_value=session_factory):
        with patch("pinaxai.storage.singlestore.inspect", return_value=MagicMock()):
...
def test_initialization():
    ...
    # Test with db_url
    with patch("pinaxai.storage.singlestore.create_engine") as mock_create_engine:
        with patch("pinaxai.storage.singlestore.sessionmaker"):
            with patch("pinaxai.storage.singlestore.inspect"):
...
def test_table_exists(agent_storage):
    ...
    with patch("pinaxai.storage.singlestore.inspect") as mock_inspect:
...
def test_mode_switching():
    ...
    with patch("pinaxai.storage.singlestore.sessionmaker"):
        with patch("pinaxai.storage.singlestore.inspect"):
            with patch("pinaxai.storage.singlestore.create_engine"):
