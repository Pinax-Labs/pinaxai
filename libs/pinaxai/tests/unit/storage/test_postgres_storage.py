...
def agent_storage(mock_engine, mock_session):
    ...
    with patch("pinaxai.storage.postgres.scoped_session", return_value=mock_session[0]):
        with patch("pinaxai.storage.postgres.inspect", return_value=MagicMock()):
...
def test_agent_storage_initialization():
    ...
    # Test with db_url
    with patch("pinaxai.storage.postgres.create_engine") as mock_create_engine:
        with patch("pinaxai.storage.postgres.scoped_session"):
            with patch("pinaxai.storage.postgres.inspect"):
...
def test_mode_switching():
    ...
    with patch("pinaxai.storage.postgres.scoped_session"):
        with patch("pinaxai.storage.postgres.inspect"):
            with patch("pinaxai.storage.postgres.create_engine"):
