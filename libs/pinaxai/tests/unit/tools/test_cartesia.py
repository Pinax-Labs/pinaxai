...
from pinaxai.agent import Agent  # Added for TTS test
from pinaxai.media import AudioArtifact  # Added for TTS test
from pinaxai.tools.cartesia import CartesiaTools

# Import the specific logger instance used by the tool
# from pinaxai.utils.log import logger as pinaxai_logger_instance # Removed unused import
...
def cartesia_tools(mock_cartesia_client):
    ...
    with patch("pinaxai.tools.cartesia.cartesia.Cartesia") as mock_cartesia_class, patch.dict(
...
def test_init_with_api_key(mock_cartesia_client):
    ...
    with patch("pinaxai.tools.cartesia.cartesia.Cartesia") as mock_cartesia_class:
        mock_cartesia_class.return_value = mock_cartesia_client
...
def test_init_missing_api_key():
    ...
    with patch("pinaxai.tools.cartesia.getenv", return_value=None), pytest.raises(ValueError):
        CartesiaTools()
