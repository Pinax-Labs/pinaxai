...
def mock_browserbase():
    ...
    with patch("pinaxai.tools.browserbase.Browserbase") as mock_browserbase_cls:
        mock_client = Mock()
        mock_sessions = Mock()
...
def mock_playwright():
    ...
    with patch("pinaxai.tools.browserbase.sync_playwright") as mock_sync_playwright:
...
def test_init_with_env_vars():
    ...
    with patch("pinaxai.tools.browserbase.Browserbase"):
        with patch.dict(
...
def test_init_with_params():
    ...
    with patch("pinaxai.tools.browserbase.Browserbase"), patch.dict("os.environ", {}, clear=True):
...
def test_init_with_missing_project_id():
    ...
    with patch.dict("os.environ", {"BROWSERBASE_API_KEY": TEST_API_KEY}, clear=True), patch(
        "pinaxai.tools.browserbase.Browserbase"
    ):
