...
def test_docx_reader_read_file(mock_docx):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        "pinaxai.document.reader.docx_reader.DocxDocument", return_value=mock_docx
    ):
        reader = DocxReader()
...
def test_docx_reader_with_chunking():
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        "pinaxai.document.reader.docx_reader.DocxDocument", return_value=mock_doc
    ):
        reader = DocxReader()
        reader.chunk = True
...
def test_docx_reader_bytesio(mock_docx):
    ...
    with patch("pinaxai.document.reader.docx_reader.DocxDocument", return_value=mock_docx):
        reader = DocxReader()
...
def test_docx_reader_file_error():
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        "pinaxai.document.reader.docx_reader.DocxDocument", side_effect=Exception("File error")
    ):
        reader = DocxReader()
        documents = reader.read(Path("test.docx"))
        assert len(documents) == 0
