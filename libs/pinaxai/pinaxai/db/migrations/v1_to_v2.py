...
from pinaxai.db.migrations.utils import quote_db_identifier
from pinaxai.db.schemas.memory import UserMemory
from pinaxai.session import AgentSession, TeamSession, WorkflowSession
from pinaxai.utils.log import log_error, log_info, log_warning
...
def get_table_content_in_batches(db: BaseDb, db_schema: str, table_name: str, batch_size: int = 5000):
    ...
    try:
        if type(db).__name__ == "MongoDb":
            from pinaxai.db.mongo.mongo import MongoDb
            ...
            if type(db).__name__ == "PostgresDb":
                from pinaxai.db.postgres.postgres import PostgresDb

                db = cast(PostgresDb, db)

            elif type(db).__name__ == "MySQLDb":
                from pinaxai.db.mysql.mysql import MySQLDb

                db = cast(MySQLDb, db)

            elif type(db).__name__ == "SqliteDb":
                from pinaxai.db.sqlite.sqlite import SqliteDb
