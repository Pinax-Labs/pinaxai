...
from pinaxai.tools.e2b import E2BTools
...
pinaxai_assist_knowledge = UrlKnowledge(
...
pinaxai_assist = Agent(
    name="Pinaxai Assist",
    role="You help answer questions about the Pinaxai framework.",
    model=OpenAIChat(id="gpt-4o"),
    instructions="Search your knowledge before answering the question. Help me to write working code for Pinaxai Agents.",
    tools=[
        KnowledgeTools(
            knowledge=pinaxai_assist_knowledge, add_instructions=True, add_few_shot=True
        ),
    ],
...
if __name__ == "__main__":
    ...
    # asyncio.run(pinaxai_assist_knowledge.aload())

    # asyncio.run(agent_team.aprint_response("Hi! What are you capable of doing?"))

    # Python code execution
    # asyncio.run(agent_team.aprint_response(dedent("""What is the right way to implement an Pinaxai Agent that searches Hacker News for good articles?
    ...
    #                                        Don't mock anything. Use the real information from the Pinaxai documentation."""), stream=True))
    ...
    # Github analysis
    # asyncio.run(agent_team.aprint_response(dedent("""List open pull requests in the Pinax-Labs/pinaxai repository.
    ...
    asyncio.run(
        agent_team.aprint_response(
            dedent(f"""I have a patient with the following medical information:\n {loaded_txt}
                                                    What is the most likely dipinaxaisis?
                                                """),
            stream=True,
        )
    )
